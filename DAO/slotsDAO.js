...
export default class slotsDAO {
  ...
  static async spin(userId, bet) {
    ...
    if (chance >= 1 && chance <= 50) {
      result = await userLost();
    }
    // win back equal amount
    else if (chance >= 51 && chance <= 70) {
      result = await userWon(userId, bet, 1);
    }
    // win 2x amount
    else if (chance >= 71 && chance <= 90) {
      result = await userWon(userId, bet, 2);
    }
    // win 3x amount
    else if (chance >= 91 && chance <= 96) {
      result = await userWon(userId, bet, 3);
    }
    // win 4x amount
    else if (chance >= 97 && chance <= 99) {
      result = await userWon(userId, bet, 4);
    }
    // win 10x amount
    else if (chance === 100) {
      result = await userWon(userId, bet, 10);
    }
